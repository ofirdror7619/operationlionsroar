...
export class PreloadScene extends Phaser.Scene {
  ...
  createNormalizedEnemyFrameTextures() {
    ...
    textureKeys.forEach((textureKey) => {
      ...
      Object.entries(frameBounds).forEach(([frameKey, bounds]) => {
        ...
        this.polishEnemyCanvasTexture(canvasTexture);
        canvasTexture.refresh();
      });
    });
  ...
  polishEnemyCanvasTexture(canvasTexture) {
    ...
    for (let i = 0; i < pixels.length; i += 4) {
      ...
      if (alpha < softEdgeCutoff) {
        alpha = Math.floor(alpha * 0.72);
        pixels[i + 3] = alpha;
      }
      ...
      pixels[i] = Phaser.Math.Clamp(Math.round((r - 128) * contrast + 128 + brightness), 0, 255);
      pixels[i + 1] = Phaser.Math.Clamp(Math.round((g - 128) * contrast + 128 + brightness), 0, 255);
      pixels[i + 2] = Phaser.Math.Clamp(Math.round((b - 128) * contrast + 128 + brightness), 0, 255);
    }
