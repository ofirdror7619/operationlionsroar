...
export class GameScene extends Phaser.Scene {
  ...
  update(_time, delta) {
    ...
    this.updateAtmosphericHudEffects(delta);
  ...
  createForegroundVignette() {
    ...
    const layers = [
      ...
      { inset: 12, alpha: 0.11 },
      { inset: 28, alpha: 0.08 },
      { inset: 48, alpha: 0.055 },
      { inset: 70, alpha: 0.04 }
    ];
    const sideBand = 62;
    const verticalBand = 46;
  ...
  ensureGrainTexture() {
    ...
    const width = 196;
    const height = 112;
    const grain = this.textures.createCanvas(key, width, height);
  ...
  createGrainOverlay() {
    const key = this.ensureGrainTexture();
  ...
  ensureWorldAtmosphereTextures() {
    if (!this.textures.exists("ambient-smoke")) {
      const width = 256;
      const height = 160;
      const smokeCanvas = this.textures.createCanvas("ambient-smoke", width, height);
      const ctx = smokeCanvas.getContext();
      ctx.clearRect(0, 0, width, height);
      for (let i = 0; i < 8; i += 1) {
        const x = Phaser.Math.Between(36, width - 36);
        const y = Phaser.Math.Between(28, height - 28);
        const radius = Phaser.Math.Between(22, 56);
        const gradient = ctx.createRadialGradient(x, y, 6, x, y, radius);
        gradient.addColorStop(0, "rgba(208, 218, 224, 0.22)");
        gradient.addColorStop(0.45, "rgba(170, 184, 194, 0.12)");
        gradient.addColorStop(1, "rgba(110, 122, 130, 0)");
    ...
    if (!this.textures.exists("dust-particle")) {
      const width = 14;
      const height = 14;
      const dustCanvas = this.textures.createCanvas("dust-particle", width, height);
      const ctx = dustCanvas.getContext();
      ctx.clearRect(0, 0, width, height);
      const gradient = ctx.createRadialGradient(7, 7, 1, 7, 7, 7);
      gradient.addColorStop(0, "rgba(241, 232, 202, 0.95)");
      gradient.addColorStop(0.5, "rgba(226, 214, 182, 0.55)");
      gradient.addColorStop(1, "rgba(226, 214, 182, 0)");
      ctx.fillStyle = gradient;
      ctx.beginPath();
    ...
    if (!this.textures.exists("heat-haze-band")) {
      const haze = this.make.graphics({ x: 0, y: 0, add: false });
      haze.fillGradientStyle(0xffd9a8, 0xffd9a8, 0xfff1d2, 0xfff1d2, 0.5);
      haze.fillRect(0, 0, 220, 54);
      haze.generateTexture("heat-haze-band", 220, 54);
      haze.destroy();
    }
  }

  createWorldAtmosphere() {
    this.ensureWorldAtmosphereTextures();
    this.createAmbientSmokeLayers();
    this.createAmbientDust();
    this.createHeatHazeBands();
  }

  createAmbientSmokeLayers() {
    const layerCount = 6;
    this.ambientSmokeLayers = [];
    for (let i = 0; i < layerCount; i += 1) {
      const smoke = this.add
        .image(
          Phaser.Math.Between(70, this.playWidth - 70),
          Phaser.Math.Between(Math.floor(this.scale.height * 0.2), Math.floor(this.scale.height * 0.66)),
          "ambient-smoke"
        )
        .setDepth(82 + i)
        .setAlpha(0.12 + i * 0.02)
        .setScale(1.05 + i * 0.18)
        .setBlendMode(Phaser.BlendModes.NORMAL);
      smoke.__driftX = Phaser.Math.FloatBetween(0.02, 0.05);
      smoke.__driftY = Phaser.Math.FloatBetween(-0.004, 0.006);
      smoke.__baseY = smoke.y;
      smoke.__phase = Phaser.Math.FloatBetween(0, Math.PI * 2);
      this.ambientSmokeLayers.push(smoke);
    }
  }

  createAmbientDust() {
    this.ambientDustEmitter = this.add.particles(0, 0, "dust-particle", {
      x: { min: 0, max: this.playWidth },
      y: { min: this.scale.height * 0.28, max: this.scale.height * 0.94 },
    ...
    this.ambientDustEmitter?.setDepth(84);
  }

  createHeatHazeBands() {
    const hazeY = this.scale.height * 0.7;
    this.heatHazeBands = [
      this.add.image(this.playWidth * 0.34, hazeY, "heat-haze-band"),
      this.add.image(this.playWidth * 0.57, hazeY + 16, "heat-haze-band"),
      this.add.image(this.playWidth * 0.78, hazeY + 8, "heat-haze-band")
    ];

    this.heatHazeBands.forEach((band, index) => {
      band
        .setDepth(28)
        .setAlpha(0.06 + index * 0.015)
        .setScale(1.05 + index * 0.15, 0.95 + index * 0.08)
        .setBlendMode(Phaser.BlendModes.ADD);
      band.__baseX = band.x;
      band.__baseY = band.y;
      band.__phase = Phaser.Math.FloatBetween(0, Math.PI * 2);
    });
  }
  ...
  updateWorldAtmosphere(delta) {
    const timeMs = this.time.now;

    this.ambientSmokeLayers.forEach((smoke) => {
      if (!smoke?.active) {
        return;
      }
      smoke.x += smoke.__driftX * delta;
      smoke.y = smoke.__baseY + Math.sin(timeMs * 0.00055 + smoke.__phase) * 6;
      smoke.alpha = 0.09 + (Math.sin(timeMs * 0.0012 + smoke.__phase) + 1) * 0.07;
      if (smoke.x > this.playWidth + 80) {
        smoke.x = -80;
      }
    });

    this.heatHazeBands.forEach((band) => {
      if (!band?.active) {
        return;
      }
      band.x = band.__baseX + Math.sin(timeMs * 0.0017 + band.__phase) * 7;
      band.y = band.__baseY + Math.cos(timeMs * 0.0019 + band.__phase) * 3;
      band.alpha = 0.035 + (Math.sin(timeMs * 0.004 + band.__phase) + 1) * 0.03;
  ...
  startAmbientCameraShake() {
    this.cameraMicroShakeEvent?.remove(false);
    this.cameraMicroShakeEvent = this.time.addEvent({
      delay: 1800,
      loop: true,
      callback: () => {
        if (this.gameOver || this.levelComplete || this.isAbortConfirmVisible()) {
        ...
        this.cameras.main.shake(160, 0.0011, true);
      }
    });
  }
